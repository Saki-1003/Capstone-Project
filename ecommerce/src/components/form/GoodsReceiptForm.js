...
import { createInventory } from "@/backend/server_actions/inventory"
import { getProducts } from "@/backend/db_query/product"
import { getInventories } from "@/backend/db_query/inventory"
import styles from './GoodsReceiptForm.module.css'

export default async function GoodsReceiptForm() {
  ...
  return(
    <main className={styles.main}> 
      <div className={styles.headingcontainer}> 
        ...
        <h1 className={styles["form-heading"]}>Good Receipt Entry</h1>   
      </div> 
      <form action={createInventory}>

        <label className={styles.labelForID} htmlFor="ProductId">Product ID *</label>
        <input className={styles.inputID} type="number" id="ProductId" name="ProductId" /><br />
              
        <label className={styles.label} htmlFor="GR_date">Goods Receipt Date *</label>
        <input className={`${styles.input} ${styles.date}`} type="date" id="GR_date" name="GR_date" /><br />
        
        <label className={styles.label} htmlFor="quantity">Quantity *</label>
        <input className={styles.input} type="number" id="quantity" name="quantity" /><br />
    
        <label className={styles.label} htmlFor="location">Location</label>
        <input className={styles.input} type="text" id="location" name="location" /><br />

        <button className={styles.btn} type="submit">Add to Inventory</button>
      </form>
    </main>
  )
}